...
function AddVoiceTextSpeechGreeting({
  formik,
  onClose,
  isLoading,
  voiceType,
  ...
  const [selectedGender, setSelectedGender] = useState('female1');
  ...
  const handleGenderChange = (e) => {
    ...
    formik.setFieldValue('voiceGender', e.target.id);
  };
  ...
  useEffect(() => {
    if (voiceType.type !== ('text-voice-convert' || 'text-voice')) {
      resetAudio();
      if (audioRef.current) audioRef.current.src = '';
      setVoicePlaying(false);
      setUrlLoading(false);
  ...
  return (
    <div className="tab-speech">
      <div
        id="textSpeechMsg"
        className={`${
          voiceType.type === 'text-voice' && voiceType.isVisible === true ? '' : 'd-none'
        }`}
      >
        <div>
            ...
            <div className="d-flex gap-3 mt-2">
              <div>
                <input
                  ...
                  name="Gender1"
                  type="radio"
                  checked={selectedGender === 'female1'}
                  onChange={handleGenderChange}
                />
                <label htmlFor="female1" className="radio-tick-label text-primary">
                  Female
                </label>
              </div>
              <div>
                <input
                  ...
                  name="Gender1"
                  type="radio"
                  checked={selectedGender === 'male1'}
                  onChange={handleGenderChange}
                />
                <label htmlFor="male1" className="radio-tick-label text-primary">
                  Male
                </label>
              </div>
            </div>
      ...
      <div
        id="voiceConvertWrap"
        className={`${voiceType.type === 'text-voice-convert' ? '' : 'd-none'}`}
      >
        <div>
          <p className="text-primary fw-medium mb-2">Text to Speech</p>
        </div>
...
export default AddVoiceTextSpeechGreeting;
