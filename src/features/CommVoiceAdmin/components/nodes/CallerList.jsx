...
function CallerList() {
  ...
  return (
    ...
    >
      <NodeHeader
        name="Caller list"
        icon="/assets/call-flows-icons/callerlist.svg"
        type="caller-list"
        actionType="edit-node"
        headerbg="cool-mint"
        iconbg="light-sky-blue"
        nodeId={nodeId}
        tooltipContent="The Caller list component helps you dynamically add each caller into or remove from a specific list."
      >
      ...
      <ul className="bg-white" style={{ borderRadius: '10px' }}>
        <li
          ...
          style={{ width: '248px' }}
          onClick={() => {
            setSelectedNode({ nodeId, type: 'after-action', sourceIndex: 1 });
          }}
        >
          ...
          <HandleSourceRight
            ...
            nodeId={nodeId}
            type="after-action"
            isVisible={isValueInArray('after-action', activeEdges)}
          />
        </li>
      </ul>
    </div>
