...
function EditAgentTimeSlot() {
  ...
  // const handleSearch = () => {
  //   const matchingNames = allHolidays?.data?.filter((holiday) =>
  ...
  //     console.log('holidayMonth ==>', holidayMonth);
  //     console.log('selectedMonth ==>', selectedMonth);

  //     if (holidayMonth === parseInt(selectedMonth, 10)) {
  //       return true;
  //     }
  ...
  const editAgentAvailability = (formik) => {
  ...
  const addHoliday = (formik) => {
    setIsLoading(true);
    // const formattedDate = moment(formik.values.holidayDate, 'DD/MM/YYYY').format('YYYY-MM-DD');

    const data = {
      type: 'telephony_holidays',
      attributes: {
        name: formik.values.holidayName,
        agent_availability_id: params?.id,
  ...
  function validate(data) {
    ...
    if (show?.type === 'update-agent-timeslot') {
      if (!data.slotName) {
      ...
      if (!data.holidayName) {
        errors.holidayName = 'please enter a holiday name';
      }
    } else if (
  ...
  const handleApplyToAllOffcanvas = () => {
    if (show?.applytype === 'apply-to-all-offcanvas') {
      ...
      ApplyToAllWorkingHourAndTimeSlot(data)
        ...
        .finally(() => {
          setIsLoading(false);
          setShow({ isVisible: false, type: '', isApplyVisible: false, applytype: '' });
        });
    }
  };
  ...
  useEffect(() => {
    setIsLoading(true);
    GetAgentAvailability(params?.id)?.then((response) => {
      formik.setFieldValue('timeZone', response?.data?.attributes?.timezone);
      formik.setFieldValue('slotName', response?.data?.attributes?.name);
  ...
  useEffect(() => {
    ...
    ListHolidays(searchTerm, params?.id, selectedMonth)?.then((response) => {
      setAllHolidays({
        data: response?.data,
  ...
  return (
    <Layout active="/comm-voice-admin/agent-availability/edit-agent-time-slot">
...
export default EditAgentTimeSlot;
