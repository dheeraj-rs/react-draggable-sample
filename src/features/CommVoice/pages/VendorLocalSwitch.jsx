...
function VendorLocalSwitch() {
      ...
      if (show?.type === 'add-switch') {
        const data = {
          ...
          attributes: {
            ...
            carrier_id: formik?.values?.carrierMapping,
    ...
    if (selectedItemsForBulkDelete?.length > 0) {
      CSVExportLocalSwitchSelected(selectedItemsForBulkDelete, type)
        .then((res) => {
          ...
          a.href = url;
          a.download = 'Local-Switch.csv'; // Set the desired file name
  ...
  useEffect(() => {
    if (isAllSelected) {
      paginatedLocalSwitchData?.data?.map((response) => {
        setSelectedItemsForBulkDelete((prevArray) => [
          ...prevArray,
          { id: parseInt(response?.id, 10), type: 'telephony_vendor_local_switches' },
        ]);
        return null;
      });
    } else {
                ...
                <div className="row">
                  ...
                  <div className="col-lg-9 col-sm-12">
                    <div className="scroll-custom scroll-carrier carrier-pad">
                      ...
                      {/* <!-- pagination --> */}
                      {paginatedLocalSwitchData?.meta?.pagination?.total > 0 && (
                        <Pagination
...
export default VendorLocalSwitch;
