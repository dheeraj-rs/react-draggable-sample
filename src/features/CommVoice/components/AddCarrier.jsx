...
function AddCarrier({
  ...
  const handleSearch = () => {
    const matchingNames = unpaginatedBatchList?.filter((batch) =>
      batch.attributes.name.toLowerCase().includes(key.toLowerCase())
    );
    setSearchResult(matchingNames);
  };

  const handleBatchIdSelection = (batchId) => {
    const index = formik?.values?.batch?.findIndex(
      (obj) => parseInt(obj.id, 10) === parseInt(batchId, 10)
    );
    if (index === -1) {
      ...
      formik?.values?.batch?.splice(index, 1);
    }
  };
        ...
        <div className="modal-dialog modal-lg modal-dialog-scrollable">
          <div className="modal-content p-lg-4 p-2 border-0">
            ...
            <div className="modal-header border-0 pb-0">
              <h4 className="modal-title text-dark fw-medium fs-15px">Add Carrier</h4>
