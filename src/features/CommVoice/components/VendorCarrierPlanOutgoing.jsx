...
import CarrierPlanListOutgoing from './CarrierPlanListOutgoing';
...
function VendorCarrierPlanOutgoing({
  paginatedCarrierOutgoingData,
  handlePaginationFunctionOutgoing,
  isFilterAppliedOutgoing,
  setIsFilterAppliedOutgoing,
  setShow,
  setSearchKeyOutgoing,
  searchKeyOutgoing,
  normalizedCarrierOutgoingData,
  handleBulkSelection,
  ...
  return (
    ...
    >
      <div className="d-flex justify-content-between align-items-center mt-lg-4 mt-3 mt-sm-4 mb-4 row ">
        <div className="col-lg-4 col-sm-4 col-md-6 col-10">
          <SearchWithBorder
            ...
            onChange={(e) => {
              setSearchKeyOutgoing(e.target.value);
            }}
            clearBtn={() => {
              setSearchKeyOutgoing('');
            }}
            searchTerm={searchKeyOutgoing}
          />
        </div>
        <div className="col-lg-4 col-sm-4 col-md-2 col-2 filter-col">
          <div id="roleSelection" className={isFilterAppliedOutgoing ? 'd-none' : ''}>
            <a
              href="/#"
              ...
              <div className="d-flex flex-column mt-3">
                ...
                <div className="setting-buttons d-flex align-items-end mt-4">
                  <button
                    id=""
                    type="button"
                    className="applyBtn btn bg-black d-flex align-items-center justify-content-center text-white px-4 py-12px"
                    onClick={() => {
                      setIsFilterAppliedOutgoing(true);
                    }}
                  >
                    Apply
                  </button>
                  <a
                    ...
                    type="button"
                    id=""
                    className="roleCancel d-flex align-items-center justify-content-center btn btn-outline-dark border fw-medium border-gray-blue px-4 py-12px ms-3"
                  >
                    Clear
                  </a>
                </div>
          ...
          <div id="selectedRole" className={isFilterAppliedOutgoing ? '' : 'd-none'}>
            <a
              href="/#"
              className="p-10px rounded text-blue-active border border-blue-active position-relative"
              onClick={(e) => {
                e.preventDefault();
                setIsFilterAppliedOutgoing(false);
              }}
            >
              ...
              <span className="filter-text">Filter</span>
              <img id="" className="clearFilter ps-0 ps-md-4" src="/assets/close-blue.svg" alt="" />
            </a>
          </div>
        </div>
      ...
      <div
        ...
        className={paginatedCarrierOutgoingData?.isLoading ? '' : 'd-none'}
      >
        <SpinningLoader />
      </div>
      {paginatedCarrierOutgoingData?.data?.length > 0 &&
        paginatedCarrierOutgoingData?.data?.map((plan, index) => (
          <CarrierPlanListOutgoing
            key={index}
            carrierPlan={plan?.attributes?.name}
            planGroup={
              normalizedCarrierOutgoingData?.carrierGroup?.[plan?.attributes?.carrier_group_id]
                ?.attributes?.name
            }
            planDate={
      ...
      {paginatedCarrierOutgoingData?.data?.length === 0 && <NoMatchingRecords />}
      {paginatedCarrierOutgoingData?.meta?.pagination?.total > 0 && (
        <Pagination
          handlePagination={handlePaginationFunctionOutgoing}
          currentPage={paginatedCarrierOutgoingData?.meta?.pagination?.current_page}
          totalPages={paginatedCarrierOutgoingData?.meta?.pagination?.total_pages}
          count={paginatedCarrierOutgoingData?.meta?.pagination?.per_page}
        />
      )}
    </div>
  );
...
export default VendorCarrierPlanOutgoing;
